...
class VMwareHostVirtualSwitch(PyVmomi):
    def __init__(self, module):
        ...
        if not hosts:
            self.module.fail_json(msg="Unable to find hosts.")

        desired_host_system = None
        if esxi_hostname:
            for host_system_obj, host_system_name in iteritems(hosts):
                if host_system_name == esxi_hostname:
    ...
    def state_update_vswitch(self):
        ...
        vss_spec = vim.host.VirtualSwitch.Specification()
        if remain_pnic:
            vss_spec.bridge = vim.host.VirtualSwitch.BondBridge(nicDevice=remain_pnic)

        if vswitch_pnic_info['mtu'] != self.mtu:
            vss_spec.mtu = self.mtu

        if vswitch_pnic_info['num_ports'] != self.number_of_ports:
            vss_spec.numPorts = self.number_of_ports
