    ...
    def ensure(self):
        ...
        for host in self.hosts:
            ...
            if state == 'reboot-host' and not host.capability.rebootSupported:
                self.module.fail_json(msg="Current host '%s' can be rebooted as the host system"
                                          " does not have capability to reboot." % host.name)
            elif state == 'shutdown-host' and not host.capability.shutdownSupported:
                self.module.fail_json(msg="Current host '%s' can be shut down as the host system"
                ...
                self.module.fail_json(msg="Current host '%s' can be %s as the host system"
                                          " does not have capability to standby supported." % host.name)
